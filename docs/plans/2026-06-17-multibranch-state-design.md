...
- **In-flight 生產變更的順序**（cache-miss B-0 / C-0）：
  - **開 release**（從 develop）探到任何未併入 main 的 `release/*` 或 `hotfix/*` → **中斷**。release 必須等所有未完成的生產變更（產品有已知 bug 在 hotfix 時不發 release；也不同時開兩個 release）。
  - **hotfix** 優先權最高，只需等**另一個**未完成的 `hotfix/*`；不因 in-flight release 中斷（那個 release 會等此 hotfix）。
  - 合法掛起的分支靠自己的 cache hit resume，本 guard 只擋「開新 flow」，不擋 resume。
  - 中斷時告知使用者先處理（合併或刪除）該分支後再 `/gitf`，不接續、不另開 `-2`。
- **合併衝突**：停下回報，由使用者解決。
- **探測結果互相矛盾**：停下回報。
